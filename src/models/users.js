...
const getUserByUsername = async (username) => {
  const [userRaw] = await pool.query('SELECT * FROM users WHERE username = ?', [username])
...
const incrementTokenVersion = async (userId) => {
  ...
  logger.info(`incrementTokenVersion: Incremented token_version for user=${userId}`)
}

export { createUser, getUserById, getUserByUsername, incrementTokenVersion }
